...
List<UserRecord>? orderByDistance(
...
int subtractTwoNumbers(
  ...
  // write a code to subtract two integers from variables and return the difference
  return num1 - num2;
}
...
int addTwoNumbers(
  ...
  // write a code to add two integers together from variables and return the sum

  int sum = num1 + num2;
  return sum;
}

List<UserRecord> getPlacesMaximumDistance(
  List<UserRecord> places,
  LatLng userGeo,
  double maxDistance,
) {
  // First create some emptyList
  List<UserRecord> placesList = [];
  List<double> listKm = [];
  double lat1 = userGeo.latitude;
  double lon1 = userGeo.longitude;
  // This iterates through the single documents "places" in the List
  for (UserRecord place in places) {
...
int? convertStringtoInt(String? convertToInt) {
...
int? yardstoKilometer(String? yards) {
  // convert a string in yards to an integer in kilometer
  if (yards == null) {
    return null;
  }
  double yardsDouble = double.tryParse(yards) ?? 0.0;
  double kilometers = yardsDouble * 0.0009144;
  return kilometers.toInt();
}

DateTime? datethirtyDaysAgo(DateTime? currentDate) {
  // Fetch the date 30 Days ago given the current date
  if (currentDate == null) return null;
  return currentDate.subtract(Duration(days: 30));
}
